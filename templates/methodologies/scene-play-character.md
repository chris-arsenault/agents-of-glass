...
4. Resolve uncertainty before prose. If a discovery attempt, social push,
   ...
   `--open-question`.
5. Persist any durable character-side changes before prose: character state,
   inventory, messages, or `players/<id>/secrets/` edits. Commit authored
   markdown with `glass sync apply`.
6. Write public turn prose to the `TURN.md` path from TURN_START. Put the
...
7. End the turn with `glass turn end`. Include the compact continuity summary,
   ...
   `none`, and `--next default` unless an override is required.
...
must see. Players do not end scenes; message the DM if the scene seems
complete.
