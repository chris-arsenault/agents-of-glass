...
|---|-------|----------------------|------------------|-------------|
| A1 | Scene framing discipline, DM + player variants (`_scene_framing_discipline_section`): scene engine must not be witnesses/evidence/audits; preflight questions | Legal-drama / procedural-legitimacy drift — scenes became about proving and recording | No positive genre direction anywhere; system-continuity framing leaked into fiction premise | **drop** — replaced by positive genre direction in the DM/player system prompts (M2) and the adversarial premise/antagonist gates (M3). Highest-priority M4 watch item. |
