"""FalkorDB connection helpers for the glass CLI.

The live runtime uses FalkorDB for neutral `(:Fact)` continuity and DB-backed
`(:LoreEntry)` reference prose. Legacy entity helpers remain for checkpoint
compatibility and non-agent maintenance code; they are not the agent state
contract.
...
def load_falkor_config(toml_data: dict[str, Any] | None = None) -> FalkorConfig:
    ...
    if not host:
        raise ValueError(
            "FalkorDB host is not configured; set [falkordb].host "
            "or AOG_FALKOR_HOST."
        )
    raw_port = (
