"""Detached process manager for the local read-only web API."""
...
from .web_api_server import DEFAULT_WEB_API_URL


RUNTIME_DIR = Path(os.environ.get("AOG_WEB_API_RUNTIME_DIR", "/tmp/agents-of-glass/web-api"))
...
def start_daemon(
    *,
    ...
    """Start the web API daemon unless a healthy one is already running."""
...
def stop_daemon(*, url: str = DEFAULT_WEB_API_URL) -> ApiDaemonInfo:
    """Stop the known web API daemon, if present."""

    stopped: list[int] = []
    for pid in _candidate_pids(url):
        if pid in stopped:
            continue
        if not _pid_matches_web_api_server(pid):
            continue
        _terminate_pid(pid)
        stopped.append(pid)
...
def status_daemon(*, url: str = DEFAULT_WEB_API_URL) -> ApiDaemonInfo:
    health = _health(url)
    if health is not None:
        return _info(
            running=True,
    ...
    if pid and _pid_matches_web_api_server(pid):
        return _info(
            running=True,
            pid=pid,
            url=url,
...
def _wait_for_health(url: str, proc: subprocess.Popen) -> dict[str, Any]:
    ...
    while time.time() < deadline:
        if proc.poll() is not None:
            raise RuntimeError(
                f"glass web API daemon exited early with {proc.returncode}; see {LOG_FILE}"
            )
        health = _health(url)
        if health is not None:
            return health
        time.sleep(0.05)
    raise RuntimeError(f"glass web API daemon did not become healthy at {url}; see {LOG_FILE}")
...
def _pid_matches_web_api_server(pid: int) -> bool:
    cmdline = _proc_cmdline(pid)
    if not cmdline:
        return False
    joined = " ".join(cmdline)
    return "web-api" in cmdline and "serve" in cmdline and (
...
def _daemon_env() -> dict[str, str]:
    ...
    The web API daemon is operator-owned and may outlive the command that started
