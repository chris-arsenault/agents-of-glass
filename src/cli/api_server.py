"""Local HTTP API for agent-facing `glass` CLI command proxying."""
...
def ensure_background_server(
    *,
    ...
    """Start the command API in this process unless something healthy exists."""
...
def _make_server(host: str, port: int) -> ThreadingHTTPServer:
    return ThreadingHTTPServer((host, port), _GlassCommandApiHandler)
...
def _wait_for_health(url: str) -> None:
    ...
    while time.time() < deadline:
        if is_server_available(url):
            return
        time.sleep(0.05)
    raise RuntimeError(f"glass command API did not become healthy at {url}")


class _GlassCommandApiHandler(BaseHTTPRequestHandler):
    server_version = "glass-command-api/0.1"
