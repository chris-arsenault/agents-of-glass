...
2. **A persistent corpus** that captured what happened: NPCs encountered, locations visited, beats advanced, threads progressed.
...
- **Prefer reversible over robust.** Easy to change the agent personalities, the mode set, the mechanics. Hard to change the transcript schema or durable state shape — design those for evolution.
