...
- **Player private:** their journal directory (free-form, **journal-shaped**, no schema, no size limit, may have subdirectories), their drafts directory (**encyclopedia-shaped** lore-in-progress), and messages addressed specifically to them via `glass msg`. **Visible to the DM** — the DM can see what every player is writing.
- **DM-only:** secret notes, the planning workspace (journal-shaped, free-form), and the intake of player-drafted lore awaiting ratification.
