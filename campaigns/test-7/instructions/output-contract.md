...
Write your final public turn prose to the `TURN.md` path named in
`TURN_START.md`, then run `glass turn end`, then exit.
...
the table, scene summary, per-turn closeout summaries, character state,
lore/journal/notes, messages, rolls, and command audit. Use the public prose to
show the story beat a viewer or future reader should experience.
...
transcripts in that file. The orchestrator reads only `TURN.md` for public
prose.
...
private planning, and conditional branches belong in messages, table state,
notes, or summaries. When the turn is primarily scene play, let the visible
fiction carry most of the prose and tuck any table-process notes after the beat.
...
Use `--next <agent-id>` only when you are overriding the normal next actor.
Use `--open-question`, `--position`, and `--pressure` when those fields changed.
