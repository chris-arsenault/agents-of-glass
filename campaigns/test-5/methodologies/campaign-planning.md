...
The campaign-root `context.md` is what every player sees during play. You will
update it as the campaign evolves. Treat it as the campaign's living
...
For the player-facing `context.md`: two short paragraphs naming the Question,
the Scarcity, the broad setting, and the tone. Plus what the players' party
knows about where they are in the world right now. **Do not expose the
antagonist map, major mystery chain, intended opening arc structure, or hidden
campaign solution in `context.md` before character creation.**
...
These are DM-facing ingredients. Character creation should hang primarily from
the setting, organization, public lore, and the player's own character concept.
If you surface hooks before character creation, expose only broad public
pressure or rumor-level situations; do not hand players the campaign's mystery
board.
